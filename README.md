...
## TODO
1.Automatically read translation modules, custom entity models, animations...
2.Translate the remaining waystone container GUI
3.Compatible with more mod GUIs
4.Bedrock inventory texture custom models and handheld item custom models
5.Compatible with more types of polymer blocks (doors, trapdoors, stairs, ... )
6.Other polymer furniture decorations besides filament blocks have not been tested

## User Guide
Put the bedframe into the mods folder, then start the server. 
After the server has fully started, shut down the server. 
In the config folder in the root directory of the server, find the bedframe folder and enter it. 
Inside, create two folders: one named converted-models and the other named polymer. 
Place the nexo model .yml configuration file from filament-nexo into the converted-models folder, and put the .zip resource pack generated by polymer into the polymer folder in bedframe. 
Then start the server again, and everything is done!